...
int main() {
    int pipe_fd;
    int res;
    int data_to_write[] = {5, 15, 25, 35}; // First program's data
    ...
    while (1) {
        // First, write to FIFO
        printf("First Program Process %d writing to FIFO\n", getpid());
        ...
        // Then, read from FIFO
        printf("First Program Process %d waiting to read from FIFO\n", getpid());
        pipe_fd = open(FIFO_NAME, O_RDONLY);
        if (pipe_fd != -1) {
            res = read(pipe_fd, data_to_read, size);
            if (res > 0) {
                printf("First Program Process %d read: %d, %d, %d, %d\n",
